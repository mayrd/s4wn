...
**Status:** S235 · 736 tests · WASM 308.0KB — Clippy: 0 errors, 0 warnings. 0 open issues. Investigated data[87]+[118] WASM duplicate: Confirmed 4559/4815 bytes (94.7%) identical prefix — serde_json flt2dec formatting tables. data[118] tail = standard library error strings ("user-provided comparison function..."). Multiple serde_json::from_str call sites with different types (JsonMesh vs Value vs Vec<u32>) cause separate monomorphized paths, each carrying data tables. LTO/codegen-units=1 already enabled — source-level consolidation needed. Actionable fix: replace serde_json in model.rs with manual JSON parser to eliminate one serde path (est. 4.8KB savings). Next: (1) Convert from_name to integer discriminant lookup (est. 15-19KB savings), (2) Model JSON dedup data[87]+[118] (est. 4.8KB), (3) Profile render() code bloat (39.7KB). 2 fog_color/sky_color sync validation tests added — validates fog matches sky ramp at horizon, fog_color uniform consistency in fragment shader — Rust-side validation of GLSL sin+smoothstep formula for sky lighting. WASM size at 308.0KB (8.0KB gap to 300KB target). Next: (1) Convert from_name to integer discriminant lookup (est. 15-19KB savings — largest single win), (2) Model JSON dedup data[87]+[118] (est. 4.8KB), (3) Profile render() code bloat (39.7KB).**
...
Next session priorities: (1) Replace serde_json in model.rs with manual JSON parser — the model JSON format is simple (version, vertices, normals, uvs, indices, aabb, material arrays); eliminates one serde monomorphization path, deduplicating data[87]+[118] (est. 4.8KB savings). See S235 findings for byte-level analysis. (2) Convert from_name() to integer discriminant lookup — eliminates 19.4KB building/unit name strings (data[5]). (3) Profile render() code[278]=39.7KB for bloat reduction. WASM target: 300KB (8KB remaining).
