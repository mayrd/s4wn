...
Status: S277 · 793 tests · Clippy: 0 errors, 0 warnings. 0 open issues. WASM baseline: 296.8KB (cargo clean build, +1.0KB from S276 caustics). All 10 name arrays are #[cfg(test)]-only. Next: (1) Migrate S4 resource/terrain name parsing in parse_map_json from string match to integer keys (data[3]: 16.2KB — ~9KB shaders + ~7KB resource/terrain strings + match code). (2) Consider alternative to serde_json data[66]=8.8KB encoding tables. (3) Phase 8: sound effects system (ambient water, weather, building sounds).
