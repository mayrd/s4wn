...
**Status:** S236 · 736 tests · WASM 300.1KB — Clippy: 0 errors, 0 warnings. 0 open issues. Replaced serde_json in model.rs with manual JSON parser — eliminated serde_json::from_str::<JsonMesh> monomorphization path, deduplicating data[87]+[118]. WASM 308.0KB → 300.1KB (-7.9KB), essentially hitting the 300KB target. The manual parser handles the fixed model JSON schema: version, vertices, normals, uvs, indices, aabb, optional material. Supports both minified and pretty-printed JSON. All 736 tests pass, clippy clean. Next: (1) Convert from_name() to integer discriminant lookup (est. 15-19KB savings — largest remaining single win), (2) Profile render() code bloat (39.7KB), (3) Verify WASM under 300KB after future size regressions.**
...
Next session priorities: (1) Convert from_name() to integer discriminant lookup — eliminates 19.4KB building/unit name strings (data[5]). Largest remaining single win. (2) Profile render() code bloat — the render() function in lib.rs is 39.7KB. (3) Monitor WASM size to stay under 300KB — we are at 300.1KB and any new code will push us over unless we find further savings.
