...
| P48 | 2026-07-17 | **Fix Camera — Prevent Perspective Change Underneath Terrain Plane** — Added `upperBetaLimit = Math.PI / 2 - 0.1` to `TouchCameraController.configure()` to prevent the ArcRotateCamera from rotating below the terrain plane (beta > PI/2 would show the underside of the ground). TypeScript checks clean, 574 unit tests green. |
