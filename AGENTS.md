...
Status: S283 · 806 tests · Clippy: 0 errors, 0 warnings. 0 open issues. WASM baseline: 255.8KB (S283 manual JSON parser). serde_json still used in network.rs (NetworkMessage to_string/from_str only — concrete type, minimal monomorphization). Next: (1) Phase 8: sound effects system (ambient water, weather, building sounds). (2) Migrate get_building_summary / get_unit_summary JSON to typed structs. (3) Investigate remaining WASM data[3]=15.7KB (shader source + building name strings in sav-parser). (4) Replace serde_json in network.rs with manual serialization for NetworkMessage.
