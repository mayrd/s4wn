...
**Status:** S256 · 771 tests · Clippy: 0 errors, 0 warnings. 0 open issues. Migrated showBuildCostTooltip() + refreshBuildingAffordability() from string-based get_build_cost(rustName) to integer-based get_build_cost_by_id(discriminant) using new BUILDING_DISCRIMINANT_BY_CONFIG_ID mapping (45 config IDs). Both functions now skip unmapped config IDs (24 decorative/Zierobjekte buildings). Pure JS changes — no WASM rebuild needed. Next: (1) Migrate placement code try_place_building to use integer discriminants (needs new WASM export). (2) Once all JS consumers use integer discriminants, remove BUILDING_NAMES/RESOURCE_NAMES/NATION_NAMES/UNIT_KIND_NAMES const arrays from WASM for ~20KB+ savings. (3) Migrate remaining JS consumers to integer discriminant lookups. (4) Re-baseline WASM size after name-string removal. (5) Continue Phase 7 rendering quality improvements.**
...
Next session priorities: (1) Migrate try_place_building() to accept integer discriminants (needs new try_place_building_by_id WASM export + JS migration). (2) Migrate JS unit consumers to use UNIT_NAMES_BY_ID integer lookup exclusively. (3) Once all JS consumers use integer discriminants, remove RESOURCE_NAMES/BUILDING_NAMES/NATION_NAMES/UNIT_KIND_NAMES const arrays from WASM binary for ~20KB+ savings. (4) Re-baseline WASM size after name-string removal. (5) Continue Phase 7 rendering quality improvements. (2) Migrate JS building consumers to use BUILDING_NAMES_BY_ID + BUILDING_ICONS_BY_ID with integer key lookups (no string-based key matching). (3) Migrate JS unit consumers to use UNIT_KIND_NAMES via integer discriminant lookup. (4) Once all JS consumers use integer discriminants, remove RESOURCE_NAMES/BUILDING_NAMES/NATION_NAMES/UNIT_KIND_NAMES const arrays from WASM binary for ~20KB+ savings. (5) Re-baseline WASM size and measure savings from name-string removal.
