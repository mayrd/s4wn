...
Status: S275 · 791 tests · Clippy: 0 errors, 0 warnings. 0 open issues. WASM baseline: 302.8KB (cargo clean build). All 10 name arrays are #[cfg(test)]-only. Next: (1) Phase 7: water surface shader improvement (specular highlight tuning, caustics). (2) Migrate data[3] S4 building names from sav-parser to JS-side lookup tables (est. ~3-5KB savings). (3) Consider alternative to serde_json remaining data segments (data[66]=8.8KB encoding tables). (4) Phase 8: sound effects system (ambient water, weather, building sounds).
