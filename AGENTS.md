...
Status: S285 · 815 tests · Clippy: 0 errors, 0 warnings. 0 open issues. serde and serde_json fully removed. get_building_summary/get_unit_summary migrated to typed structs. Next: (1) Phase 8: sound effects system (ambient water, weather, building sounds). (2) Investigate remaining WASM data[3]=15.7KB (shader source + building name strings in sav-parser). (3) Rebuild WASM and measure size reduction from serde removal. (4) Migrate get_building_at_tile from JSON string to typed struct (has malformed JSON bug). (5) Migrate get_resource_counts_by_id / get_stats from JSON to typed return.
