...
Status: S269 · 786 tests · Clippy: 0 errors, 0 warnings. 0 open issues. Gated NationType::NAMES + from_name behind #[cfg(test)] — both only used in test code (BalanceResult simulation). Converted BalanceResult.nation_name: &str → nation: NationType integer discriminant. Last remaining name array now gated. All 10 name arrays are #[cfg(test)]-only. Next: (1) Rebuild WASM and baseline size after removing last name string from production path. (2) Phase 7: shadow map blur pass. (3) Phase 7: water surface shader improvement. (4) Use twiggy to audit remaining non-render WASM bloat.
