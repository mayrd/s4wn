...
Status: S268 · 786 tests · Clippy: 0 errors, 0 warnings. 0 open issues. Fix #77: terrain sprites in Object Explorer. All terrain entries now have spriteFile in config. _buildDetailCache uses config as primary sprite source for terrain. WASM 296KB baseline (no change — pure JS/data). Next: (1) Use twiggy to audit remaining non-render WASM bloat. (2) Phase 7: shadow map blur pass. (3) Phase 7: water surface shader improvement. (4) Investigate NationType::NAMES→ID conversion for BalanceResult.
