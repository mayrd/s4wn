...
1. **Resource Transport Visualization** — Render carriers (donkeys/porters) moving between buildings along computed paths.
2. **Building Ghost Preview** — Wire `BuildingPlacement` pointer events to scene picking so a semi-transparent building preview follows cursor before placement confirmation.
3. **Building Placement Integration** — Connect `BuildingPlacement` to GameApp: on `building-placed` event, create 3D BuildingMesh at the correct terrain position via scene picking coordinates.
