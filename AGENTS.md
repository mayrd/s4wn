...
Status: S276 · 793 tests · Clippy: 0 errors, 0 warnings. 0 open issues. WASM baseline: 302.8KB (cargo clean build). All 10 name arrays are #[cfg(test)]-only. Next: (1) Migrate data[3] S4 building names from sav-parser to JS-side lookup tables (est. ~3-5KB savings). (2) Consider alternative to serde_json remaining data segments (data[66]=8.8KB encoding tables). (3) Phase 8: sound effects system (ambient water, weather, building sounds). (4) Re-baseline WASM size after caustics shader code addition.
