...
Status: S260 · 773 tests · Clippy: 0 errors, 0 warnings. 0 open issues. Removed dead try_place_building (string-based) WASM export + replaced BUILDING_NAMES references with discriminants - all string-based WASM building consumers now eliminated. Migrated get_player_nation() from name string to name_id integer + JS-side NATION_NAMES_BY_ID lookup. BUILDING_NAMES (77 strings) and NationType::NAMES (5 strings) now only referenced in test code; LTO will strip from WASM on next rebuild. Next: (1) Rebuild WASM and measure size savings from BUILDING_NAMES + NAMES removal. (2) Eliminate RESOURCE_NAMES from restore_game_state backward-compat fallback (convert input/output buffer parsing to integer keys). (3) Eliminate UNIT_KIND_NAMES from military_in_rect() - convert to integer return or gate behind cfg(test). (4) Re-baseline WASM size target after all name-string removals.
