...
Status: S278 · 793 tests · Clippy: 0 errors, 0 warnings. 0 open issues. WASM baseline: 296.8KB (S277 caustics baseline). All 10 name arrays are #[cfg(test)]-only. Next: (1) Re-baseline WASM after parse_map_json string removal (cargo clean + wasm-pack build) to measure savings. (2) Consider alternative to serde_json data[66]=8.8KB encoding tables. (3) Phase 8: sound effects system (ambient water, weather, building sounds). (4) Migrate get_tile_at terrain/resource output from string names to integer discriminants. (5) Remove get_tile_at JSON format strings (last remaining terrain/resource name strings in WASM).
