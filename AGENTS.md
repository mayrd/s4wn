...
Status: S264 · 776 tests · Clippy: 0 errors, 0 warnings. 0 open issues. S264 gates all remaining test-only name arrays: UNIT_KIND_NAMES + UNIT_STATE_NAMES (S262) + BUILDING_NAMES (87) + RESOURCE_NAMES (29) + UNIT_SPECIAL_NAMES (5) + SPECIALIST_NAMES (6) + TOOL_NAMES (11). 176 name-array entries gated. OnlyAMES + UNIQUE_BUILDING_NAMES remain un-gated (used in production). Next: (1) Build baseline WASM (-d) to measure name-string elimination gain. (2) Phase 7 rendering — shadow map blur pass, water surface shader improvement. (3) Investigate UNIQUE_BUILDING_NAMES→ID mapping possibility (48 entries, ~4KB). (4) Audit for any other test-only code bloat in WASM.
