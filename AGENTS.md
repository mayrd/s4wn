...
**Status:** S255 · 771 tests · Clippy: 0 errors, 1 warning (pre-existing needless_range_loop in test). 0 open issues. Audit confirmed: no remaining name() methods (WorkerKind never existed as an enum; all ResourceType/BuildingType/NationType/UnitKind name() methods already removed in S250–S254). Added get_build_cost_by_id(discriminant: u8) WASM export — integer-discriminant-based cost lookup using BuildingType::from_discriminant(). JS imports added, cache v62. 2 new tests (all_discriminants round-trip + invalid rejection). Pure Rust + WASM — ready for JS migration from string-based get_build_cost(). Next: migrate JS showBuildCostTooltip() and placement code to use get_build_cost_by_id().**
