...
Status: S267 · 786 tests · Clippy: 0 errors, 0 warnings. 0 open issues. All 9 name arrays now #[cfg(test)]-gated (only NationType::NAMES remains in production for BalanceResult). Mine spark particles added for ore mines (Gold/Coal/IronOre). WASM 296KB clean-build baseline (294.8→296KB from particle code). Next: (1) Use twiggy to audit remaining non-render WASM bloat (data segments 3,66,0). (2) Phase 7: shadow map blur pass. (3) Phase 7: water surface shader improvement. (4) Investigate NationType::NAMES→ID conversion for BalanceResult (5 strings, ~0.5KB).
