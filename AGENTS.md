...
1. **Resource Item 3D Rendering** — Render physical resource items on the ground (small colored box/sphere per ResourceType) that disappear when picked up. Integrate with SupplyChainRenderer to show resource items at their spawn positions.
2. **Marketplace & Trade Routes** — Implement donkey-based land trade between Marketplaces: trader units carry goods between player Marketplace buildings, ship-based maritime routes via LandingDock/Shipyard.
