...
```
1. **Marketplace & Trade Routes** — Implement donkey-based land trade between Marketplaces: trader units carry goods between player Marketplace buildings, ship-based maritime routes via LandingDock/Shipyard.
2. **StorageYard Carrier Routing Priority** — Carriers should prioritize delivering resources to StorageYards (central storage) over delivering directly to consumer buildings, enabling the 8-stack logistics pattern from BASE.md.
3. **Refine Production Chains** — Ensure all building production chains from BASE.md are fully modeled (input requirements, output rates, settler assignments per nation variant).
4. **Resource Item Pickup Animation** — Add a brief scale-down/fade animation when resource items are picked up by carriers, providing visual feedback for the logistics system.
