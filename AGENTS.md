...
Status: S262 · 773 tests · Clippy: 0 errors, 0 warnings. 0 open issues. Gated UNIT_KIND_NAMES (48 entries, ~4.5KB) + UNIT_STATE_NAMES (8 entries) + military_in_rect() behind #[cfg(test)] — eliminates ~4.9KB from WASM (309.3→304.4KB). All 5 name arrays (BUILDING_NAMES, RESOURCE_NAMES, NATION_NAMES, UNIT_KIND_NAMES, UNIT_STATE_NAMES) now #[cfg(test)]. Next: (1) Re-baseline WASM size target after all name-string removals — new baseline 304.4KB. (2) Migrate set_player_nation from name string to discriminant integer (JS-side NATION_NAMES_BY_ID already exists). (3) Audit for any remaining #[cfg(test)] gaps in non-test array declarations. (4) Phase 7 rendering — shadow map blur pass, water surface shader improvement.
