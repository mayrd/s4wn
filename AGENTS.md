...
**Status:** S259 · 773 tests · Clippy: 0 errors, 0 warnings. 0 open issues. Removed get_build_cost (string-based) WASM export — all JS consumers migrated to get_build_cost_by_id in S255–S257. 17 lines removed. Next: (1) Remove BUILDING_NAMES/RESOURCE_NAMES/NATION_NAMES/UNIT_KIND_NAMES const arrays from WASM for ~20KB+ savings — now that all name() methods and string-based WASM consumers are gone, the const arrays themselves can be eliminated. (2) Re-baseline WASM size after name-string removal. (3) Continue Phase 7 rendering quality improvements. (4) Remove from_name() lookup tables — only needed in tests now, can be converted to integer-based.**
...
Next session priorities: (1) Migrate remaining JS string-based building consumers to integer discriminants — get_build_cost callers, building summary consumers, etc. (2) Once all JS consumers use integer discriminants, remove BUILDING_NAMES/RESOURCE_NAMES/NATION_NAMES/UNIT_KIND_NAMES const arrays from WASM binary for ~20KB+ savings. (3) Re-baseline WASM size after name-string removal. (4) Continue Phase 7 rendering quality improvements. (5) Migrate get_build_cost (string-based) callers to get_build_cost_by_id, then remove get_build_cost.
