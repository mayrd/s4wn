...
**Status:** S257 · 773 tests · Clippy: 0 errors, 0 warnings. 0 open issues. Added try_place_building_by_id WASM export using BuildingType::from_discriminant() — integer-based building placement without string parsing. Migrated JS placement code: selectBuilding() now uses BUILDING_DISCRIMINANT_BY_CONFIG_ID to set placementMode as u8 discriminant; click handler calls try_place_building_by_id(); icon lookup uses BUILDING_ICONS_BY_ID; status text uses buildingName(disc). Bumped cache v62→v63. WASM rebuilt. Next: (1) Migrate remaining JS string-based building consumers to integer discriminants. (2) Once all JS consumers use integer discriminants, remove BUILDING_NAMES/RESOURCE_NAMES/NATION_NAMES/UNIT_KIND_NAMES const arrays from WASM for ~20KB+ savings. (3) Re-baseline WASM size after name-string removal. (4) Continue Phase 7 rendering quality improvements. (5) Migrate get_build_cost (string) callers to get_build_cost_by_id.**
