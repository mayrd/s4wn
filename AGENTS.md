...
Status: S282 · 793 tests · Clippy: 0 errors, 0 warnings. 0 open issues. WASM baseline: 276.6KB (S282 Vec<u32> migration). All 10 name arrays are #[cfg(test)]-only. serde_json still used in parse_map_json (Value) + network.rs (NetworkMessage). Next: (1) Replace parse_map_json serde_json::Value with manual JSON parser (est. 8-10KB savings). (2) Phase 8: sound effects system (ambient water, weather, building sounds). (3) Migrate get_building_summary / get_unit_summary JSON to typed structs. (4) Investigate remaining WASM data[3]=15.7KB (shader source + building name strings in sav-parser).
