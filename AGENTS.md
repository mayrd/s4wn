...
Status: S286 · 817 tests · Clippy: 0 errors, 0 warnings. 0 open issues. get_building_at_tile migrated to typed struct (malformed JSON bug fixed). Next: (1) Phase 8: sound effects system (ambient water, weather, building sounds). (2) Investigate remaining WASM data[3]=15.7KB (shader source + building name strings in sav-parser). (3) Rebuild WASM and measure size reduction from serde removal. (4) Migrate get_resource_counts_by_id / get_stats from JSON to typed return. (5) Fix malformed JSON in get_building_info (line 4149: "kind":{}" missing quote).
