...
Next session priorities: (1) ~~Replace BuildingType.name() callers with direct NAMES array access, then remove BuildingType.name() method.~~ ✅ (S251) (2) ~~Replace NationType.name() callers with direct NAMES array access, then remove NationType.name() method.~~ ✅ (S252) (3) Audit remaining name() methods (UnitKind, WorkerKind, UnitState) and convert to integer-only consumers. (3) After BuildingType/NationType name() removal, audit remaining name() methods (UnitKind, WorkerKind, UnitState) and convert to integer-only consumers. (4) Migrate JS building-related consumers to use BUILDING_NAMES_BY_ID + BUILDING_ICONS_BY_ID for integer key lookups (no more string-based key matching). (5) Once ALL name() methods removed from economy.rs/nation.rs/units.rs, ~20KB+ WASM strings can be deleted from RESOURCE_NAMES/BUILDING_NAMES/NATION_NAMES const arrays for target WASM savings.
