...
Next session priorities: (1) Replace ResourceType.name() with .discriminant() in get_resource_counts(), get_building_info(), get_build_cost(), get_game_state() JSON output. RESOURCE_NAMES_BY_ID (19 entries) already in data.js — add RESOURCE_ICONS_BY_ID for emoji lookup. Add get_resource_counts_by_id() WASM export + JS consumer. (2) Replace BuildingType.name() with .discriminant() in lib.rs JSON output functions — get_game_state(), get_building_summary(), get_building_info(), try_place_building(), get_nation_buildings(). BUILDING_NAMES_BY_ID (77 entries) already in data.js; update JS consumers to use kind_id + lookup fallback. (3) Add BUILDING_ICONS_BY_ID map alongside BUILDING_NAMES_BY_ID for emoji lookup by discriminant. (4) After all JS consumers use discriminants, remove name() methods from ResourceType + BuildingType + NationType for ~19.4KB WASM savings.
