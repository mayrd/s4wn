...
1. **Building Ghost Preview** — Wire `BuildingPlacement` pointer events to scene picking so a semi-transparent building preview follows cursor before placement confirmation.
2. **Building Placement — Click-on-Terrain** — Replace hardcoded (50,50) placement in `BuildingPlacement.onPointerDown()` with actual scene picking coordinates from the canvas pointer event.
