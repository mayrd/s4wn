...
## Purpose
This folder is the complete asset pack for the **Mayans** faction in S4WN.
...
├── models/                # 3D meshes
│   ├── buildings/         # Mayans-themed building OBJ/GLB meshes
│   ├── decorations/       # Border posts, flags, and other scenery
│   └── units/             # Mayans-themed unit GLB/OBJ meshes
...
This is the single source of truth for the Mayans pack. It defines:
...
(`assets/nations/mayans/`).

## Validation
Loaded at startup by `src/game/NationLoader.ts` and validated by
`src/game/NationValidator.ts`. The Mayans pack must pass all validation rules.
