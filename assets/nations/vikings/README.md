...
## Purpose
This folder is the complete asset pack for the **Vikings** faction in S4WN.
...
├── models/                # 3D meshes
│   ├── buildings/         # Vikings-themed building OBJ/GLB meshes
│   ├── decorations/       # Border posts, flags, and other scenery
│   └── units/             # Vikings-themed unit GLB/OBJ meshes
...
This is the single source of truth for the Vikings pack. It defines:
...
(`assets/nations/vikings/`).

## Validation
Loaded at startup by `src/game/NationLoader.ts` and validated by
`src/game/NationValidator.ts`. The Vikings pack must pass all validation rules.
