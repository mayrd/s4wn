...
## Purpose
This folder is the complete asset pack for the **Trojans** faction in S4WN.
...
├── models/                # 3D meshes
│   ├── buildings/         # Trojans-themed building OBJ/GLB meshes
│   ├── decorations/       # Border posts, flags, and other scenery
│   └── units/             # Trojans-themed unit GLB/OBJ meshes
...
This is the single source of truth for the Trojans pack. It defines:
...
(`assets/nations/trojans/`).

## Validation
Loaded at startup by `src/game/NationLoader.ts` and validated by
`src/game/NationValidator.ts`. The Trojans pack must pass all validation rules.
